...
public class AttackTriggerC : MonoBehaviour {
    ...
    {
        c = 0;
        NormalTrigger(); // just to activate the animations on the mechanim character . 
    }
	void Update(){
		...
		if(Input.GetButton("Fire1")){
            if (Time.time > nextFire && !isCasting && !mobileMode && !skillAim) {
                NormalTrigger();
            }
        }
    ...
    private void NormalTrigger()
    {
        if (Time.time > (nextFire + 0.5f)) {
            c = 0;
            Debug.Log("mouse basic fire");
        }
        //Attack Combo
    ...
    public int rangeAttacks { get; private set; }
