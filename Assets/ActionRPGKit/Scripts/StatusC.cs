...
public class StatusC : MonoBehaviour {
	...
	public void Death(){
		...
		if(gameObject.CompareTag("Player")){
		    ...
		    CustomDataTracker.onMosterKilled();
		}
		Destroy(gameObject);
		if(deathBody){
            //transform.GetChild(3).GetComponent<Animator>().SetBool("Die",true);
            Vector3 corpsePosition = new Vector3(transform.position.x, transform.position.y, transform.position.z);  //y = 1.68
			Instantiate(deathBody, corpsePosition , transform.rotation);
		}else{
